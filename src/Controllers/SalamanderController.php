...
class SalamanderController
{
  ...
  private function validateId(int $id): bool
  {
    if (!is_numeric($id) || $id <= 0) {
      ...
      echo '<p><a href="/salamanders">Back to list</a></p>';
      return false;
    }
    return true;
  }
  ...
  public function store(): void
  {
    ...
    if (empty($name) || empty($habitat) || empty($description)) {
      ...
      echo '<p><a href="/salamanders/create">Back to form</a></p>';
      return;
    }
    ...
    if ($id > 0) {
      ...
      header("Location: /salamanders/show?id=$id");
      exit;
    } else {
  ...
  public function update(int $id): void
  {
    ...
    if (empty($name) || empty($habitat) || empty($description)) {
      ...
      echo '<h1>400 Bad Request</h1>';
      echo '<p>All fields are required.</p>';
      echo '<p><a href="/salamanders/edit?id=' . htmlspecialchars($id) . '">Back to form</a></p>';
      return;
    }
  ...
  public function destroy(int $id): void
  {
    ...
    if ($success) {
      ...
      header("Location: /salamanders");
      exit;
    } else {
      http_response_code(500);
      echo '<h1>500 Server Error</h1>';
      echo '<p>Failed to delete salamander. Please try again.</p>';
      echo '<p><a href="/salamanders/delete?id=' . htmlspecialchars($id) . '">Back</a></p>';
      return;
    }
  }
}
